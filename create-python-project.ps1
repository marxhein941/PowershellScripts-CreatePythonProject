...
# -------------------------------------
# Updated script to output to docs folder
# -------------------------------------

# Get the directory of the script
$scriptDirectory = Split-Path -Parent $MyInvocation.MyCommand.Definition

# Go TWO levels up from the script's directory, to reach the actual project root
$rootFolder = [System.IO.Path]::GetFullPath("$scriptDirectory\..\..")

# Create a "docs" folder in that root directory
$docsFolder = Join-Path $rootFolder "docs"

# The name of the text file we'll generate
...
if (-not (Test-Path $docsFolder)) {
    Write-Host "Creating 'docs' folder in root..."
    try {
    ...
    Write-Host "'docs' folder already exists in root."
}
...
# Generate the folder structure via 'tree', filtering out pycache/.pyc
...
try {
    ...
    $filteredTreeOutput | Out-File -FilePath $outputFile
